        ...
        private void WriteMessage(long messageId)
        {
            ...
            if (message != null)
            {
                ...
                if (isRecipient || isSender || this.CommandResult.CurrentUser.IsAdministrator)
                {
                    ...
                    this.CommandResult.WriteLine(DisplayMode.Inverted | DisplayMode.DontType, "{{{0}}} {1}{2}", messageId, messageLocked ? "[LOCKED] " : null, message.Subject);
                    this.CommandResult.WriteLine(DisplayMode.DontType, "sent to {0} from {1} on {2}", message.Recipient, message.Sender, message.SentDate);

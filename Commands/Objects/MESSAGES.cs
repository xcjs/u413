        ...
        private void WriteMessages(string username, int page, bool sent)
        {
            ...
            if (user != null)
            {
                ...
                if (page > messagesPage.TotalPages)
                    ...
                    page = 1;
                this.CommandResult.WriteLine(DisplayMode.Inverted | DisplayMode.DontType, "{0} messages for {1}", sent ? "Sent" : "Received", user.Username);
                ...
                foreach (var message in messagesPage.Items)
                {
                    ...
                    this.CommandResult.WriteLine(displayMode, "{{{0}}} {1}{2}", message.MessageID, messageLocked ? "[LOCKED] " : null, message.Subject);
                    this.CommandResult.WriteLine(displayMode, "   sent {0} {1} {2}", sent ? "to" : "from", sent ? message.Recipient : message.Sender, message.SentDate.TimePassed());
                    this.CommandResult.WriteLine();
                }
